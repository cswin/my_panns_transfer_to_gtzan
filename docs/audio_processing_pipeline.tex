...
\node[box, below=of stft] (stft_out) {STFT Output\\(97, 513)};
\node[box, below=of stft_out] (mel) {Mel Filterbank};
\node[box, below=of mel] (mel_out) {Mel-Spectrogram\\(97, 64)};
...
% Labels
\node[right=0.5cm of stft_out] {97 time frames $\times$ 513 frequency bins};
\node[right=0.5cm of mel_out] {97 time frames $\times$ 64 mel bins};
...
\begin{enumerate}
    ...
    \item \textbf{STFT Output}: Creates 2D matrix of frequency content over time
    \item \textbf{Mel Filterbank}: Compresses frequency dimension using perceptual scaling
    \item \textbf{Mel-Spectrogram}: Final 2D representation for machine learning
\end{enumerate}
...
\node[box, right=of input] (output) {STFT Matrix\\(97 $\times$ 513)};
...
\node[box] (stft) {STFT: (97, 513)\\\small{513 linear frequency bins}};
\node[box, right=of stft] (mel) {Mel: (97, 64)\\\small{64 mel-scaled frequency bins}};
