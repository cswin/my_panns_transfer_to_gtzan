...
def test_target_matched_steering():
    """Test TARGET-MATCHED steering where each sample uses its own emotion category's steering signal."""
    ...
    # Load steering signals (NEW PAIRS format)
    steering_signals_path = './steering_signal_pairs_25bin/steering_signal_pairs_25bin.json'
    ...
    # Check if we loaded the new pairs format and count categories properly
    excluded_keys = {'metadata', 'generation_config'}
    category_count = len([k for k in signals_25bin.keys() if k not in excluded_keys])
    
    if 'metadata' in signals_25bin and 'extraction_method' in signals_25bin.get('metadata', {}):
        method = signals_25bin['metadata']['extraction_method']
        print(f"✅ Loaded steering signals: {category_count} categories (Method: {method})")
        if method == 'separate_pathway_averaging':
            print("   🎯 Using NEW properly-separated valence/arousal signal pairs!")
    else:
    ...
    # Analyze what categories we have
    print("\n🔍 Analyzing target categories in dataset...")
    category_counts = {}
    ...
    for category, count in sorted(category_counts.items()):
        available = "✅" if category in signals_25bin and category not in excluded_keys else "❌"
        print(f"   {available} {category}: {count} samples")
    
    # Test different strengths
    strengths = [0.0, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0]
    results = {}
    
    for strength in strengths:
        print(f"\n🔍 Testing TARGET-MATCHED steering with strength: {strength}")
        ...
        for sample_idx, (features, true_val, true_aro, audio_name) in enumerate(real_data):
            ...
            # Prepare steering signals (TARGET-MATCHED)
            if strength > 0.0 and target_category in signals_25bin and target_category not in excluded_keys:
    ...
    # Compare results
    print(f"\n📊 TARGET-MATCHED Results Comparison:")
    ...
    if best_val_improvement > 0.01 or best_aro_improvement > 0.01:
        print("   🎉 SUCCESS: Target-matched steering improves predictions!")
    else:
        print("   ⚠️  MARGINAL: Target-matched steering shows small effects")

if __name__ == "__main__":
    test_target_matched_steering() 