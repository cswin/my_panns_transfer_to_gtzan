...
def test_valence_conv4_only_steering():
    """Test steering ONLY the valence conv4 layer (most specific targeting)."""
    ...
    if 'metadata' in signals_9bin and 'binning_system' in signals_9bin.get('metadata', {}):
        ...
        print("   🎯 Using 9-bin steering signals with VALENCE-CONV4-ONLY targeting!")
    else:
    ...
    for strength in strengths:
        print(f"\n🔍 Testing VALENCE-CONV4-ONLY steering with strength: {strength}")
        ...
        for sample_idx, (features, true_val, true_aro, audio_name) in enumerate(real_data):
            ...
            if strength > 0.0 and target_category in signals_9bin and target_category not in excluded_keys:
                ...
                if 'valence_128d' in signals:
                    ...
                    # Target ONLY conv4 layer (valence pathway) - most specific targeting
                    # We'll use the standard steering mechanism but it should only affect conv4
                ...
                if sample_idx < 3:  # Debug first few samples
                    ...
                    print(f"         Steering: VALENCE-CONV4-ONLY, targeting only conv4 layer")
            else:
    ...
    # Compare results
    print(f"\n📊 VALENCE-CONV4-ONLY Results Comparison:")
    ...
    if best_val_improvement > 0.01:
        print("   🎉 SUCCESS: Valence-conv4-only steering improves valence predictions!")
    elif best_val_improvement > -0.01:
        print("   ✅ GOOD: Valence-conv4-only steering maintains valence performance!")
    else:
        print("   ⚠️  MARGINAL: Valence-conv4-only steering shows small effects")
    
    print(f"   Note: Targeting only conv4 layer for maximum specificity")

if __name__ == "__main__":
    test_valence_conv4_only_steering() 