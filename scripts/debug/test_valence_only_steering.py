...
def test_valence_only_steering():
    """Test steering ONLY the valence target layers (conv3/conv4)."""
    ...
    if 'metadata' in signals_9bin and 'binning_system' in signals_9bin.get('metadata', {}):
        ...
        print("   🎯 Using 9-bin steering signals with VALENCE-ONLY targeting!")
    else:
    ...
    for strength in strengths:
        print(f"\n🔍 Testing VALENCE-ONLY steering with strength: {strength}")
        ...
        for sample_idx, (features, true_val, true_aro, audio_name) in enumerate(real_data):
            ...
            # Prepare steering signals (VALENCE-ONLY, targeting conv3/conv4)
            if strength > 0.0 and target_category in signals_9bin and target_category not in excluded_keys:
                ...
                if 'valence_128d' in signals:
                    ...
                    # Target conv4 layer (valence pathway) - if it exists
                ...
                if sample_idx < 3:  # Debug first few samples
                    ...
                    print(f"         Steering: VALENCE-ONLY, targeting conv3/conv4 layers")
            else:
    ...
    # Compare results
    print(f"\n📊 VALENCE-ONLY Results Comparison:")
    ...
    if best_val_improvement > 0.01:
        print("   🎉 SUCCESS: Valence-only steering improves valence predictions!")
    elif best_val_improvement > -0.01:
        print("   ✅ GOOD: Valence-only steering maintains valence performance!")
    else:
        print("   ⚠️  MARGINAL: Valence-only steering shows small effects")
    
    print(f"   Note: Arousal performance should remain unchanged (no arousal steering)")

if __name__ == "__main__":
    test_valence_only_steering() 